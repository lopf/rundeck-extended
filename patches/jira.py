...
options:
  ...
  comment_format:
    ...
    required: false
    description:
      - Format type for the comment text.
      - V(jira_markup) treats the comment as JIRA markup and uses the v2 API (default).
      - V(plain) treats the comment as plain text and requires v3 API.
      - V(code) formats the entire comment as a code block and requires v3 API.
      - V(adf) expects the comment to be a simplified ADF object structure and requires v3 API.
      - When using V(plain), V(code), or V(adf), you must explicitly set O(api_version=3).
    choices: [plain, jira_markup, code, adf]
    default: jira_markup
    version_added: '9.0.0'
  comment_visibility:
    type: dict
  ...
  api_version:
    ...
    required: false
    description:
      - Force a specific JIRA REST API version to use.
      - V(2) uses the legacy v2 API endpoints (supports only V(jira_markup) comments).
      - V(3) uses the modern v3 API endpoints (supports all comment formats).
      - For search operations, V(2) uses C(/search?jql=) while V(3) uses C(/search/jql?jql=). Note that some JIRA Cloud instances have removed the v2 search endpoint.
      - When using advanced comment formats (V(plain), V(code), V(adf)), you must explicitly set V(3) as API v2 does not support these formats.
    choices: ['2', '3']
    default: '3'
    version_added: '9.0.0'
notes:
  ...
  - For JIRA Cloud instances, some API v2 endpoints (particularly search) may be deprecated or removed. Use O(api_version=3) for full compatibility.
  - API v2 only supports V(jira_markup) comment format. Attempting to use other formats with API v2 will result in an error with guidance to use API v3.
  - When using advanced comment formats (V(plain), V(code), V(adf)), you must explicitly set O(api_version=3).
author:
  - "Steve Smith (@tarka)"
...
# Comment with JIRA markup formatting
# This is the default format, the v2 API is used. 
- name: Comment on issue with JIRA markup
  community.general.jira:
    uri: '{{ server }}'
    username: '{{ user }}'
    password: '{{ pass }}'
    ...
    comment: |
      ...
      some code block
      {code}
      And some {color:red}colored text{color}
    comment_format: jira_markup

# Comment formatted as code block with explicit API version
- name: Comment on issue with code formatting
  community.general.jira:
    uri: '{{ server }}'
    username: '{{ user }}'
    password: '{{ pass }}'
...
# Comment formatted as code block
- name: Comment on issue with code formatting
  community.general.jira:
    uri: '{{ server }}'
    username: '{{ user }}'
...
# Plain text comment
- name: Comment on issue with plain text
  community.general.jira:
    uri: '{{ server }}'
    username: '{{ user }}'
    password: '{{ pass }}'
...
# Advanced ADF formatting - JSON string
- name: Comment with complex ADF formatting
  community.general.jira:
    uri: '{{ server }}'
    username: '{{ user }}'
    password: '{{ pass }}'
...
# Advanced ADF formatting - YAML structure
- name: Comment with ADF structure as YAML
  community.general.jira:
    uri: '{{ server }}'
    username: '{{ user }}'
    password: '{{ pass }}'
...
# Search using v3 API (recommended for JIRA Cloud)
- name: Search for an issue using v3 API
  community.general.jira:
    uri: '{{ server }}'
    username: '{{ user }}'
    password: '{{ pass }}'
...
# Search using v2 API (for local JIRA installations that still support it)
- name: Search for an issue using v2 API
  community.general.jira:
    uri: '{{ server }}'
    username: '{{ user }}'
    password: '{{ pass }}'
...
# You can get list of valid linktypes at /rest/api/3/issueLinkType
...
class JIRA(StateModuleHelper):
    ...
    def _format_comment_body(self, comment_text, format_type='jira_markup', api_version='2'):
        ...
        Args:
            comment_text: The text content of the comment or ADF structure
            format_type: 'plain', 'jira_markup', 'code', or 'adf'
            api_version: API version ('2' or '3') 
        """
        # Just return the original text for jira_markup
        if format_type == 'jira_markup':
            return comment_text
        
        # For v3 API, return ADF structures
        if format_type == 'adf':
            # User provides simplified ADF structure
            if isinstance(comment_text, dict):
                ...
                try:
                    ...
                    return self._format_comment_body(comment_text, 'plain', api_version)
        
        elif format_type == 'code':
            # Format entire comment as a code block
            return {
                'type': 'doc',
                'version': 1,
                'content': [
            ...
            # If no content, add a single empty paragraph
            if not content:
                content = [{
                    'type': 'paragraph',
                    'content': [
    ...
    module = dict(
        argument_spec=dict(
            ...
            comment=dict(type='raw', ),
            comment_format=dict(type='str', choices=['plain', 'jira_markup', 'code', 'adf'], default='jira_markup'),
            ...
            api_version=dict(type='str', choices=['2', '3'], default='2', required=False),
        ),
        mutually_exclusive=[
            ['username', 'token'],
            ['password', 'token'],
    ...
    def __init_module__(self):
        ...
        self.vars.set('restbase', self.vars.uri + '/rest/api/' + self.vars.api_version)
        self.vars.set('api_version', self.vars.api_version)
        # Validate API version and comment format compatibility
        if (hasattr(self.vars, 'api_version') and self.vars.api_version == '2' and 
            self.vars.operation == 'comment' and 
            getattr(self.vars, 'comment_format', 'jira_markup') != 'jira_markup'):
            self.module.fail_json(
                msg="API version 2 only supports comment_format='jira_markup'. "
                    "For other comment formats (plain, code, adf), you must explicitly set api_version='3'."
            )
    ...
    def operation_comment(self):
        data = {
            'body': self._format_comment_body(self.vars.comment, self.vars.comment_format, self.vars.api_version)
        }
    ...
    def operation_worklog(self):
        data = {
            'comment': self._format_comment_body(self.vars.comment, self.vars.comment_format, self.vars.api_version)
        }
        # if comment_visibility is specified restrict visibility
        if self.vars.comment_visibility is not None:
            data['visibility'] = self.vars.comment_visibility
    ...
    def operation_search(self):
        # Use different search endpoints based on API version
        if self.vars.api_version == '2':
            # v2 API uses /search?jql=
            url = self.vars.restbase + '/search?jql=' + pathname2url(self.vars.jql)
        else:
            # v3 API uses /search/jql?jql=
            url = self.vars.restbase + '/search/jql?jql=' + pathname2url(self.vars.jql)
    ...
    def operation_attach(self):
        ...
        url = self.vars.restbase + '/issue/' + v.issue + '/attachments'
